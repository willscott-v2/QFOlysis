...
export async function performContentAnalysis(
  ...
  try {
    ...
    return {
      ...
      radarData,
      coverageGaps,
      recommendations: generateOverallRecommendations(coverageGaps, targetCategoryScores),
      queries: allQueries,
...
function generateOverallRecommendations(
  ...
  if (highPriorityGaps.length > 0) {
    recommendations.push(
      `Priority: Address content gaps in ${highPriorityGaps.map(g => g.category).join(', ')} categories`
    );
  }

  // Add strength-based recommendations
  const strongCategories = categoryScores.filter(s => s.score > 80);
  if (strongCategories.length > 0) {
    recommendations.push(
      `Leverage your strengths in ${strongCategories.map(s => s.category).join(', ')} for competitive advantage`
    );
  }

  // Add general recommendations
  const averageScore = categoryScores.reduce((sum, s) => sum + s.score, 0) / categoryScores.length;
  if (averageScore < 50) {
    recommendations.push('Consider a comprehensive content audit to improve overall coverage');
  } else if (averageScore > 75) {
    recommendations.push('Maintain content quality while exploring new topic areas');
  }
